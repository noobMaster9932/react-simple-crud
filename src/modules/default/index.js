...
export class index extends Component {
  ...
  render() {
    const { isLoading, getData, dataGrid } = this.props
    const { modal } = this.state
    return (
      <Fragment>
        { modal.status && <ModalDetail data={modal.data} close={() => this._toggleModal(false)} /> }
        <Row>
                  ...
                  {item.attachment !== null && (
                    <div className='text-center mb-2'>
                      <img
                        ...
                        onClick={() => this._toggleModal(true,item)}
                      />
                    </div>
                  )}
                  <div className='mb-2' 
  ...
  _toggleModal = (status, data = {}) => this.setState({ modal:{ status, data } })
}
